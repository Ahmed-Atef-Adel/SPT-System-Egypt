import { fromEvent, window } from 'rxjs';
import * as sidebarFn from '../sidebar/sidebar'
import { Direction, Language, LanguageService } from '../../services/language.service';
...
export function LangChange(translateService: TranslateService) {
  ...
  let body = document.querySelector('body');
  let html = document.querySelector('html');
  const langservice = new LanguageService(translateService);
  // LTR
...
export function customClickFn(translateService: TranslateService) {
  const langservice = new LanguageService(translateService);
  let body = document.querySelector('body');
  let html = document.querySelector('html');
  let styleId = document.querySelector('#style');
  ...
  fromEvent(TransparentBtn, 'click').subscribe(() => {
    localStorage.setItem('currentTheme', 'transparent');
